# this code is download from probhap
...
def get_clouds(start, end, clouds_at_index):
    
    clouds = set() # debug should be set not list
...
class Cloud():
    ...
    def __getslice__(self,s,e): # directly [s,e), yanbo
        '''
        start, end = self.start, self.end
        print self.start, end, s, e
        if s >= start and e-1 <= end: 
            return self.seq[s-start : e-start]
        else:
